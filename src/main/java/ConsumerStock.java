...
public class ConsumerStock {
    ...
    public static void main(String[] args) {
        ...
        Logger logger = LoggerFactory.getLogger(ConsumerStock.class);
        ...
        kafkaConsumer.subscribe(Arrays.asList("btn"));

        // Empezaremos a leer

        CountDownLatch countDownLatch = new CountDownLatch(1);

        final Runnable kafkaThread = new ConsumerThreadStock(countDownLatch, kafkaConsumer);

        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
                logger.info("parando gracefully");
                ((ConsumerThreadStock) kafkaThread).shutdown();
